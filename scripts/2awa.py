...
def parse_args():
    ...
    parser.add_argument('--inner_iter_num', type=int, default=8000, help='the inner iteration number')
    ...
    parser.add_argument('--data_dir', type=str, default='/data/ZSL/datasets/AwA2', help='the train directory')
    parser.add_argument('--lamb', type=float, default=0.3, help='the balance parameter')
    parser.add_argument('--alpha', type=float, default=1e-4, help='the balance parameter')
    # parser.add_argument('--gzsl', type=bool, default=False, help='enable generalized ZSL')
    parser.add_argument('--T', type=int, default=3, help='the temperature')
    parser.add_argument('--outer_lr', type=float, default=1e-6, help='the outer learning rate')
...
class Model(object):
    ...
    def create_model(self):
        ...
        with tf.name_scope('image_channel'):
            ...
            prediction = tf.nn.log_softmax(logits, axis=-1)
        ...
        with tf.name_scope('classifier'):
            ...
            self.predict = tf.nn.log_softmax(predict, axis=-1)
        ...
        with tf.name_scope('inner_loss'):
            ...
            self.inner_loss = loss_a + kl_loss + 1e-3*regularisers
