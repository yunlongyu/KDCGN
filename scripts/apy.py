...
def parse_args():
    ...
    parser.add_argument('--outer_iter_num', type=int, default=500, help='the outer iteration number')
    parser.add_argument('--inner_iter_num', type=int, default=10000, help='the inner iteration number')
    parser.add_argument('--batch_size', type=int, default=256, help='the number of batch_size')
    parser.add_argument('--hid_dim', type=int, default=800, help='the dimensionality of hidden layer')
    parser.add_argument('--data_dir', type=str, default='/data/ZSL/datasets/APY', help='the train directory')
    parser.add_argument('--lamb', type=float, default=2, help='the balance parameter')
    parser.add_argument('--alpha', type=float, default=1e-2, help='the balance parameter')
    parser.add_argument('--outer_lr', type=float, default=1e-4, help='the outer learning rate')
    parser.add_argument('--inner_lr', type=float, default=5e-5, help='the inner learning rate')
    parser.add_argument('--T', type=int, default=4, help='the temperature')
...
class Model(object):
    ...
    def create_model(self):
        ...
        with tf.name_scope('outer_loss'):
            ...
            regularisers = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])
            cross_entropy = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=prediction, labels=self.lab_pl))
            self.outer_loss = cross_entropy + base_loss + 1e-3*regularisers
        ...
        with tf.name_scope('inner_loss'):
            ...
            self.inner_loss = loss_a + kl_loss + self.alpha * base_loss + 1e-3 * regularisers
    ...
    def train(self):
        ...
        weight = self.sess.run(self.W_classifier)
        weight = weight.T
